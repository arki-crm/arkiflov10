...
'''
    ...
    def test_auto_collaborator_system(self):
        ...
        # Test stage changes with valid stages
        print("\n--- Testing Stage Changes for Auto-Collaborator ---")
        
        # Test Production stage (should trigger ProductionManager and OperationsLead)
        success, stage_response = self.run_test(
            "Update Project to Production Stage",
            "PUT",
        ...
        if success:
            # Check if project has activity and collaborators
            success, project_data = self.run_test(
                "Get Project Activity After Production",
                "GET",
            ...
            if success:
                ...
                print(f"   Collaborator activities: {len(collaborator_activities)}")
                print(f"   Total collaborators: {len(collaborators)}")
                
                # Check if any manager roles are in collaborators
                manager_roles = ["DesignManager", "ProductionManager", "OperationsLead"]
                manager_collaborators = [
                    collab for collab in collaborators 
                    if collab.get("role") in manager_roles
                ]
                
                print(f"   Manager collaborators: {len(manager_collaborators)}")
                
                return len(activity) > 0  # At least some activity should be present
    ...
    def test_activity_feed(self):
        """Test activity feed for stage changes"""
        ...
        # Get project and check for comments (which should include system comments)
        success, project_data = self.run_test(
            "Get Project with Comments/Activity",
            "GET",
            f"api/projects/{project_id}",
            200,
            auth_token=self.tokens["Admin"]
        ...
        if success:
            # Check if comments array exists (system comments are stored here)
            ...
            if len(comments) > 0:
                # Check for system comments (stage changes)
                system_comments = [
                    comment for comment in comments 
                    if comment.get("is_system", False)
                ]
                
                print(f"   System comments (stage changes): {len(system_comments)}")
                
                # Verify system comment structure
                if len(system_comments) > 0:
                    first_system = system_comments[0]
                    has_required_fields = all(
                        field in first_system 
                        for field in ["id", "user_id", "user_name", "message", "is_system", "created_at"]
                    )
                    print(f"   System comment structure valid: {has_required_fields}")
                    
                    return has_comments and has_required_fields
    ...
    def run_all_tests(self):
        ...
        print(f"   1. User Invite (9 roles): {'✅ PASS' if test1_result else '❌ FAIL'}")
        print(f"   2. Dashboard Access: {'✅ PASS' if test2_result else '❌ FAIL'}")
        print(f"   3. Auto-Collaborator: {'✅ PASS' if test3_result else '❌ FAIL'}")
        print(f"   4. Role Restrictions: {'✅ PASS' if test4_result else '❌ FAIL'}")
        print(f"   5. Activity Feed: {'✅ PASS' if test5_result else '❌ FAIL'}")
